...
namespace MamaFit.API.Controllers
{
    ...
    public class WarrantyRequestController : ControllerBase
    {
        ...
        [HttpGet("branch")]
        public async Task<IActionResult> GetAllWarrantyRequestOfBranch(
            [FromHeader(Name = "Authorization")] string accessToken, [FromQuery] int index = 1,
            [FromQuery] int pageSize = 10, [FromQuery] string? search = null, [FromQuery] EntitySortBy? sortBy = null)
        {
            var result =
                await _warrantyRequestService.GetAllWarrantyRequestOfBranchAsync(index, pageSize, search, sortBy,
                    accessToken);
